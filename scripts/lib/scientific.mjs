/**
 * Espelho de arquitetura-arduino-node-api/backend/public/js/scientific.js
 * adaptado para execucao no Node (collectEnvironment usa process em vez de navigator).
 * Mantenha em sincronia com o frontend para preservar o formato de export.
 */
...
export function createRunSummary({ experiment, samples, invalidMessages, sequenceGapMessages = 0 }) {
  ...
  const elapsedSeconds = Math.max(
    (Date.parse(experiment.stoppedAt ?? new Date().toISOString()) -
      Date.parse(experiment.startedAt)) /
      1000,
    1
  );
...
export function formatClockSyncExport(clockSync) {
  ...
  return {
    ...
    arduinoToFrontendOffsetMs: clockSync.arduinoToFrontendOffsetMs ?? null,
    rttMs:
      clockSync.backendToFrontendRttMs ??
      clockSync.frontendBackendRttMs ??
      clockSync.arduinoToBackendRttMs ??
      null,
    uncertaintyMs: clockSync.arduinoToFrontendUncertaintyMs ?? null,
