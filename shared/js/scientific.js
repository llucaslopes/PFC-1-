...
/**
 * Modulo cientifico compartilhado entre os DOIS frontends do projeto:
 *   - `prototypes/webserial/js/scientific.js`
 *   - `arquitetura-arduino-node-api/backend/public/js/scientific.js`
 *
 * Sub-fase 3.1 / clean code: antes deste arquivo as duas copias divergiam
 * em alguns campos (applicationVersion, fallback chains em
 * `formatClockSyncExport`, parametro `options` em `createDownloadFilename`,
 * export de `sanitizeFilenamePart`).
 *
 * REGRA DE PARIDADE BIT-A-BIT DOS OUTPUTS:
 *   - Cada call site no consumidor X (webserial ou backend client) precisa
 *     produzir EXATAMENTE o mesmo output que produzia antes da unificacao.
 *   - Por isso unimos as cadeias de fallback (`?? ?? ??`) preservando a
 *     precedencia que cada consumidor ja usava. Como os campos exclusivos
 *     de cada lado sao `undefined` do outro, a unificacao e idempotente
 *     para cada consumidor.
 *   - `SCIENTIFIC_CONFIG.applicationVersion` e um placeholder ("0.0.0")
 *     sobrescrito pelos wrappers (cada frontend tem versionamento proprio).
 *
 * Sincronizacao: o consumidor importa este modulo de `./_shared/scientific.js`,
 * que e mantido em sync com este arquivo pelo `scripts/sync-shared-frontend.mjs`.
 * Source of truth: `shared/js/scientific.js`.
 */
...
export function formatClockSyncExport(clockSync) {
  ...
  // Cadeias de fallback unificadas: cada consumidor preenche apenas um
  // subconjunto dos campos; os demais sao `undefined` e a precedencia
  // cobre AMBAS as ordens originais (webserial e backend client).
  return {
    ...
    rttMs:
      clockSync.arduinoToFrontendRttMs ??
      clockSync.arduinoHostRttMs ??
      clockSync.backendToFrontendRttMs ??
      clockSync.frontendBackendRttMs ??
      clockSync.arduinoToBackendRttMs ??
      null,
    uncertaintyMs:
